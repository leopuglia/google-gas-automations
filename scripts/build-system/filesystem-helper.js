...
export function ensureBuildBeforeDeploy(config, paths, projectKey = null, forceBuild = false) {
  ...
  for (const key of projectKeys) {
    ...
    // Usar o valor de output em vez de src para verificar o diretório de build
    // O output é o nome do diretório onde os arquivos compilados são armazenados
    const outputDir = projectConfig.output || key;
    const buildDir = path.join(paths.build, outputDir);

    logger.debug(`Verificando diretório de build para o projeto ${key}: ${buildDir}`);

    // Verificar se o diretório existe e se tem arquivos dentro
    ...
    // Verificar se o diretório tem arquivos dentro
    const files = fs.readdirSync(buildDir);
    if (files.length === 0) {
      logger.debug(`Diretório de build vazio para o projeto ${key}: ${buildDir}`);
      needsBuild = true;
      break;
    }
  }
